    ...
    subscriber.messageEventCb = function (session, message) {
        var payload;
            if (message.getType() == solace.MessageType.BINARY) {
                ...
                subscriber.log('Topic:[' + message.getDestination() + '] ' + 'binary message: "' + message.getBinaryAttachment() + '"');
//                subscriber.log('Received binary message: "' + message.getBinaryAttachment() + '"' + 'on topic: ' + message. );
            } else if (message.getType() == solace.MessageType.TEXT) {  // in case someone sends text message
                payload = message.getSdtContainer().getValue();
                subscriber.log("invalid message: TEXT attachment");
            } else {
                subscriber.log("invalid message: OTHER attachment");
            }
    };
